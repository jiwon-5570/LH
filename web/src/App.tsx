  ...
  return <Page title="AI 보고서" subtitle="서울·자치구·단지 단위의 운영 DB 근거를 스냅샷으로 저장하고 HTML/PDF 보고서를 생성합니다.">
    ...
    <div className="report-result">
      ...
      <div className="report-grid">
        ...
        <section className="panel"><div className="panel-head"><h2><ShieldCheck/> 6. 점검 우선 권고</h2></div><ol className="report-list report-recommendations">{created.recommendations.length?created.recommendations.map((x:any)=><li key={x.priority}><b>{x.priority}</b><div><strong>{x.text}</strong><small>근거: {x.reason}</small></div></li>):<li><b>—</b><div><strong>현재 활성 근거 기반 추가 권고 없음</strong><small>새로운 관측 또는 점검 결과 확보 후 재평가합니다.</small></div></li>}</ol></section>
      </div>
