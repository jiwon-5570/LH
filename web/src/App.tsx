...
function Reports({complexes}:{complexes:Complex[]}) {
  ...
  const reportRows=scopeType==="seoul"?complexes:scopeType==="district"?complexes.filter(x=>x.district===district):complexes.filter(x=>x.complex_id===target);
  return <Page title="AI 보고서" subtitle="서울·자치구·단지 단위의 운영 DB 근거를 스냅샷으로 저장하고 HTML/PDF 보고서를 생성합니다.">
    ...
    <div className="report-result">
      <section className="panel report-title"><div><span className="eyebrow">LH-PREDICT · {created.report_type_label}</span><h2>AI {created.scope_type==="complex"?"단지 정밀":"회복력 종합"} 보고서 <em>{created.scope.label}</em></h2><p>보고서 생성 {new Date(created.generated_at).toLocaleString("ko-KR")} · 분석 기준 {created.reference_time?new Date(created.reference_time).toLocaleString("ko-KR"):"데이터 부족"} · 버전 {created.report_version}</p></div><div className="report-downloads"><a href={created.html_download_url}><FileText/> HTML</a><a className="primary" href={created.pdf_download_url}><FileText/> PDF 다운로드</a></div></section>
      <div className="report-kpis report-kpis-five"><ReportKpi icon={<Building2/>} label="분석 단지 수" value={`${summary.total_complexes}개`} note={`${created.scope.label} 분석 범위`} color="#2575eb"/><ReportKpi icon={<Activity/>} label="평균 Re:Safe 점수" value={summary.average_resilience==null?"미분석":`${summary.average_resilience}점`} note="100점 만점 운영 선별지수" color="#22a06b"/><ReportKpi icon={<TriangleAlert/>} label="취약 단지" value={`${summary.vulnerable}개`} note={`${summary.total_complexes?Math.round(summary.vulnerable/summary.total_complexes*1000)/10:0}%`} color="#e5484d"/><ReportKpi icon={<Siren/>} label="주의 단지" value={`${summary.caution}개`} note={`${summary.total_complexes?Math.round(summary.caution/summary.total_complexes*1000)/10:0}%`} color="#e6a700"/><ReportKpi icon={<Database/>} label="데이터 부족" value={`${summary.insufficient}개`} note="추가 근거 확보 필요" color="#7c4dff"/></div>
      <div className="report-visual-grid"><section className="panel"><div className="panel-head"><h2>1. Re:Safe 점수 분포</h2><span>실제 분석 점수 기준</span></div><ReportDistribution summary={summary}/></section><section className="panel report-map-compact"><div className="panel-head"><h2>2. 분석 범위 지도</h2><span>NAVER Maps · 검증 좌표만 표시</span></div><NaverMap rows={reportRows} onSelect={()=>{}}/></section></div>
      <div className="report-grid">
        <section className="panel report-ai"><div className="panel-head"><h2><BrainCircuit/> 3. AI 종합 해설</h2><span>검증 DB 근거만 사용</span></div><p className="report-explanation">{created.ai_explanation}</p></section>
        <section className="panel"><div className="panel-head"><h2><Activity/> 4. 핵심 지표</h2></div><dl className="report-metrics"><div><dt>평균 Re:Safe</dt><dd>{summary.average_resilience??"데이터 부족"}</dd></div><div><dt>분석 가능</dt><dd>{summary.analysis_available}개</dd></div><div><dt>보통</dt><dd>{summary.normal}개</dd></div><div><dt>양호</dt><dd>{summary.good}개</dd></div></dl>{created.comparison?.selected!=null&&<div className="comparison-strip"><b>선택 단지 {created.comparison.selected}점</b><span>자치구 평균 {created.comparison.district_average??"미분석"}</span><span>서울 평균 {created.comparison.seoul_average??"미분석"}</span></div>}</section>
      </div>
      <div className="report-grid">
        <section className="panel"><div className="panel-head"><h2><Search/> 5. 주요 발견사항</h2></div><div className="report-findings">{created.findings.length?created.findings.map((x:any,i:number)=><article key={i}><span>{i%3===0?<Droplets/>:i%3===1?<Activity/>:<Building2/>}</span><div><b>{i+1}. 근거 기반 발견</b><p>{x.text}</p></div></article>):<article><span><ShieldCheck/></span><div><b>추가 발견 없음</b><p>현재 데이터에서 확정 가능한 추가 발견사항이 없습니다.</p></div></article>}</div></section>
        <section className="panel"><div className="panel-head"><h2><ShieldCheck/> 6. 점검 우선 권고</h2></div><ol className="report-list report-recommendations">{created.recommendations.length?created.recommendations.map((x:any)=><li key={x.priority}><b>{x.priority}</b><div><strong>{x.text}</strong><small><em>선정 이유</em>{x.reason}</small></div></li>):<li><b>—</b><div><strong>현재 활성 근거 기반 추가 권고 없음</strong><small>새로운 관측 또는 점검 결과 확보 후 재평가합니다.</small></div></li>}</ol></section>
      </div>
      <div className="report-grid"><section className="panel"><div className="panel-head"><h2><TriangleAlert/> 7. 취약 단지 TOP 10</h2><span>Re:Safe 점수 오름차순</span></div><div className="table"><div className="tr th"><span>단지명</span><span>자치구</span><span>등급</span><span>점수</span></div>{created.ranking.map((x:any)=><div className="tr" key={x.complex_id}><b>{x.complex_name}</b><span>{x.district||"미확인"}</span><span className="badge" style={{color:riskColor(x.score)}}>{x.grade}</span><strong>{x.score}</strong></div>)}</div></section><section className="panel"><div className="panel-head"><h2><FileText/> 8. 분석 방법론</h2><span>산정 방식과 AI 역할</span></div><dl className="method-table"><div><dt>분석 범위</dt><dd>{created.scope.label}</dd></div><div><dt>Re:Safe 방식</dt><dd>{created.methodology.resilience}</dd></div><div><dt>Cascade 방식</dt><dd>{created.methodology.cascade}</dd></div><div><dt>ML 상태</dt><dd>{created.methodology.ml_status}</dd></div><div><dt>AI 역할</dt><dd>{created.methodology.ai_role}</dd></div><div><dt>데이터 신뢰</dt><dd>{summary.insufficient===0?"분석 대상 전체 산정 가능":"일부 데이터 보강 필요"}</dd></div></dl></section></div>
...
function ReportKpi({icon,label,value,note,color}:{icon:React.ReactNode;label:string;value:string;note:string;color:string}) { return <article className="report-kpi"><span style={{color,background:`${color}16`}}>{icon}</span><div><small>{label}</small><strong>{value}</strong><p>{note}</p></div></article>; }
function ReportDistribution({summary}:{summary:any}) { const groups=[{n:"취약",v:summary.vulnerable,c:"#e5484d"},{n:"주의",v:summary.caution,c:"#f2b705"},{n:"보통",v:summary.normal,c:"#4b86e8"},{n:"양호",v:summary.good,c:"#35b987"},{n:"부족",v:summary.insufficient,c:"#94a3b8"}];let cursor=0;const total=summary.total_complexes||1;const stops=groups.map(g=>{const start=cursor;cursor+=g.v/total*360;return `${g.c} ${start}deg ${cursor}deg`;}).join(",");return <div className="report-donut-area"><div className="report-donut" style={{background:`conic-gradient(${stops})`}}><div><b>{summary.total_complexes}</b><span>전체 단지</span></div></div><div className="report-donut-legend">{groups.map(g=><div key={g.n}><i style={{background:g.c}}/><span>{g.n}</span><b>{g.v}개</b><em>{(g.v/total*100).toFixed(1)}%</em></div>)}</div></div>;}
function DataManagement({quality,hydrology}:{quality:any;hydrology:any}) {
  ...
  return <Page title="데이터 관리" subtitle="API 설정과 실제 적재 데이터 상태를 구분해 확인합니다.">
    ...
    <section className="panel"><div className="panel-head"><h2>파일/API 적재 상태</h2><span>최신 canonical snapshot 기준</span></div><div className="source-grid">{available.map(([name,value]:any)=><div className="source-card" key={name}><span className={`source-dot ${value.status==="AVAILABLE"?"ready":"blocked"}`}/><div><b>{name}</b><small>{value.status} · {value.record_count??0}건 · {value.crs||"비공간"}</small>{value.blocking_reason&&<small>{value.blocking_reason}</small>}</div></div>)}</div></section>
